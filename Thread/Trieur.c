...
int main(int argc, char *argv[]) {
    ...
    cptr_thread = compteur_gc(tampon, taille);
    assert(clock_gettime(CLOCK_MONOTONIC, &fin2) != -1);

    for (int i=0;i<nb_thread;i++){
        s2.taille = taille/nb_thread ;
        s2.bloc = tampon + s2.taille*i;

        pthread_create(thr+i,NULL,my_compteur_gc,&s2);
        pthread_join (thr[i],NULL);
        
        cptr_thread +=(unsigned long) s2.res;
    }
